...
class OnereportsController extends AppController {
	...
	function index() {
		...
		if($this->data){
			...
			if($this->data['Onereport']['folio'] != 0){
				$conditions["Onereport.folio"]=$this->data["Onereport"]["folio"];
			}
		}
		if($usuario['group_id'] == 4 && $this->data['Onereport']['ingeniero'] == 0){
				...
				$ing = $this->Engineer->find('all',array('conditions'=>array('filial_id'=>$filial)));	
				$engineers = array();
				...
				$conditions["Onereport.engineer_id"] = $engineers;
				$this->Engineer->recursive = 1;
	...
	function getEmsefor(){
		...
		if ( $this->RequestHandler->isAjax() ) {
			...
			if($user["group_id"] == 2){
				...
				$emsefors = $this->Onereport->Emsefor->find("all", array("conditions"=>array("Emsefor.filial_id"=>$filial,"Emsefor.id >"=>1000,"Emsefor.nombre LIKE '%".$st."%'")));
			}
			else{	
				$ing = 	$this->Onereport->Engineer->find("all");	
				$emsefors = $this->Onereport->Emsefor->find("all",array("conditions"=>array("Emsefor.id >"=>1000,"Emsefor.nombre LIKE '%".$st."%'")));
			}
			$i = 0;
	...
	function view($id = null) {
		...
		$this->set('engineer',$this->Engineer->find('list'));
	...
	function add() {
		...
		if (!empty($this->data)) {
			...
			if ($this->Onereport->save($this->data,array("validate"=>false))) {
				...
				$this->Session->setFlash(__('La idea no pudo ser guardada. Comuníquese con el administrador.', true));
			}
		}
	...
	function edit($id = null) {
		...
		$engineers = $this->Onereport->Engineer->find("list",array('conditions'=>array('filial_id<=3')));
		$unities  = $this->Onereport->Unity->find("list");
		$emsefors = $this->Onereport->Emsefor->find("list");
		...
		else if($usuario['group_id']==3 || $usuario['group_id']==1){
			$tipoUsuario = 1;
		}
		//$projecttypes = $this->Onereport->Projecttype->find('list');
		//$tworeports = $this->Onereport->Tworeport->find('list');
		$this->set(compact('indicators', 'ideasstates', 'cartastates', 'proyectostates','engineers','unities','emsefors','positions','tipoUsuario'));
	}
	...
	function delete($id = null) {
		...
		$this->Session->setFlash(__('La idea no ha podido ser borrada. Comuníquese con el administrador', true));
		$this->redirect(array('action' => 'ver'));
	}
	...
	function buscar($proyectos = null){
		if(isset($proyectos) || isset($this->data['Onereport']['proyectoFlag'])){
			$this->set('title_for_layout','Sistema de reportes | Registro de proyectos | Buscar proyectos');
		...
		if($this->data){
			...
			if($this->data["Onereport"]["proyecto"] != 0 && isset($this->data["Onereport"]["proyecto"])){
				$conditions["Onereport.proyectostate_id"]=$this->data["Onereport"]["proyecto"];
			}
			if($this->data['Onereport']['unity'] != 0){
				$conditions["Onereport.unity_id"]=$this->data["Onereport"]["unity"];
