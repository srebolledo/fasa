...
class TworeportsController extends AppController {
	...
	function index() {
		echo $this->_cierraPlanificaciones();
		...
		foreach($tworeports as $tworeport){
			$semana[$tworeport["Tworeport"]["semana"]] = $this->_semanas($tworeport["Tworeport"]["semana"]);
	...
	function ver($id = null){
		...
		switch($id){
						...
						case 2:
							$this->set('title_for_layout','Sistema de reportes | Registro de planificación | Realizadas');
							break;
						case 3:
							$this->set('title_for_layout','Sistema de reportes | Registro de planificación | No realizadas');
							break;
						case 4:
							$this->set('title_for_layout','Sistema de reportes | Registro de planificación | Replanificadas');
							break;
					}
				}
				else{
...
/////
	...
	function edit ($id = null) {
		...
		$this->set(compact('engineers', 'activities', 'emsefors', 'unities', 'states',"id",'places'));
	}
	...
	function realizada($id = null){
		if (!$id) {
			...
			if($this->data && $this->data['Tworeport']['save'] == true){
				$this->data["Tworeport"]["state_id"] = 2;
				if($this->Tworeport->save($this->data)){
					$this->redirect(array('action'=>'ver'));
				}
			}
			
		else if($planificacion['Tworeport']['activity_id'] == 1 || $planificacion['Tworeport']['activity_id'] == 11){
				echo "tratando de guardar!";	
				$this->set('title_for_layout','Sistema de reportes | Agregar participantes a capacitación / taller');
				$this->data = $this->Tworeport->read(null, $id);
				$this->set("emsefors_id",$this->data["Tworeport"]["emsefor_id"]);
					$user = $this->Session->read("Auth.User");
			if($user["group_id"]!=1){
		...
		$save = true;
		$this->set(compact('engineers', 'activities', 'emsefors', 'unities', 'states',"id",'places','save'));
			}
			
		else{
				$planificacion["Tworeport"]["state_id"] = 2;
				if($this->Tworeport->save($planificacion)){
...
//			$eTime = strtotime("now"); // End as time
	...
	function _semanas($semana){
		$fechaIni = date('d-m-Y',strtotime('1 november 2010')+86400*7*($semana-1));
		$fechaFin = date('d-m-Y',strtotime('1 november 2010')+86400*7*($semana));
		$fechaIni = explode('-',$fechaIni);
		$fechaFin = explode('-',$fechaFin);
		$meses = array('Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre');
		return "Semana del ".$fechaIni[0]."-".$fechaIni[1]."-".$fechaIni[2]." al ".$fechaFin[0]."-".$fechaFin[1]."-".$fechaFin[2];
	}
	
	function _cierraPlanificaciones(){
		$epoch = strtotime(date('d F Y'));
		$anterior = $epoch - 86400*10;
		$dias = date('Y-m-d',$anterior);
		$usuario = $this->Session->read('Auth.User');
		$this->Engineer->recursive = -1;
		$ing = $this->Engineer->find('all',array('conditions'=>array('user_id'=>$usuario['id'])));
		$this->Tworeport->recursive = -1;
		$reportes = $this->Tworeport->find('all',array('conditions'=>array('fecha<\''.$dias.'\'','state_id'=>1,'engineer_id'=>$ing[0]['Engineer']['id'])));
		foreach($reportes as $reporte){
			$this->Tworeport->id = $reporte['Tworeport']['id'];
			$this->Tworeport->saveField('state_id',3);
		}
