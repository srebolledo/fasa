...
<?php echo $this->Form->create('Tworeport');?>
	...
	/*	if ($i++ % 2 == 0) {
		...
		if ($tworeport["Tworeport"]["state_id"] == 2){
			$class = ' class="altrow"';
		}
	...
	<tr<?php echo $class;?> onclick="window.location.assign('<?php echo $view_url; ?>');">
		...
		<td><?php echo $tworeport['Tworeport']['participantes'];?>
		<td><?php echo $tworeport['Tworeport']['participantes_reales'];?>
	...
	<!--	<?php echo $this->Html->link(__('Ver', true), array('action' => 'view', $tworeport['Tworeport']['id'])); ?>-->
			...
			<?php 
				...
				echo $this->Html->link(__('Borrar', true), array('action' => 'delete', $tworeport['Tworeport']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $tworeport['Tworeport']['id']));
